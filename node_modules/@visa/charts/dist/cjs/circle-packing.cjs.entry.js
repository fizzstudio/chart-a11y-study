...
const CirclePacking = class {
  constructor(hostRef) {
    ...
    this.setCircleFilter = (_, i, n) => {
      return !visaChartsUtils_umd.select(n[i]).classed('moving') && !this.accessibility.hideStrokes ? this.filter : null;
    };
    this.setIDs = () => {
    ...
    this.setCircleStyle = (_, i, n) => {
      const d = visaChartsUtils_umd.select(n[i].parentNode).datum();
  ...
  componentWillLoad() {
    const chartID = this.uniqueID || 'circle-pack-' + index$2.v4();
  ...
  restructureData() {
    ...
    this.innerDataDepth = this.dataDepth
      ? Math.min(max.max(this.nodes, d => d.depth), this.dataDepth)
      : max.max(this.nodes, d => d.depth);
  ...
  renderRootElements() {
    this.svg = visaChartsUtils_umd.select(this.circlePackingEl)
    ...
    this.tooltipG = visaChartsUtils_umd.select(this.circlePackingEl).select('.circle-packing-tooltip');
  }
  reSetRoot() {
  ...
  checkRecursion(depth) {
    this.updateParentCircle.each((d, i, n) => {
      ...
      if (nextDepth && d.children && d.children.length) {
        this.enterUpdateExitCircles(visaChartsUtils_umd.select(n[i]), d.children);
      }
      else {
        this.exitCircles(visaChartsUtils_umd.select(n[i]).selectAll('g'), false);
      }
    });
  }
  ...
  placeLabels() {
    ...
    const labelUpdate = this.updateText
      ...
      const oldX = +visaChartsUtils_umd.select(n[i]).attr('data-x');
      const oldY = +visaChartsUtils_umd.select(n[i]).attr('data-y');
  ...
  updateDisplayDepth() {
    this.innerDisplayDepth = this.displayDepth
      ? Math.min(max.max(this.nodes, d => d.depth), this.displayDepth)
      : max.max(this.nodes, d => d.depth);
    if (this.circleG) {
      this.circleG.selectAll('g').each((d, i, n) => {
        if (d.depth > this.innerDisplayDepth) {
          this.exitSize++;
          this.exitSize += visaChartsUtils_umd.select(n[i])
            .selectAll('g')
            .size();
          visaChartsUtils_umd.select(n[i]).remove();
        }
      });
    }
  }
  ...
  zoom(target) {
    ...
    this.circle
      ...
      const d = visaChartsUtils_umd.select(n[i].parentNode).datum();
      const oldX = +visaChartsUtils_umd.select(n[i]).attr('data-cx');
      const oldY = +visaChartsUtils_umd.select(n[i]).attr('data-cy');
      const oldR = +visaChartsUtils_umd.select(n[i]).attr('r');
      const newX = roundTo((d.x - this.view[0]) * this.zoomRatio, 4);
      const newY = roundTo((d.y - this.view[1]) * this.zoomRatio, 4);
    ...
    if (this.duration && changeCircle.size() > 0) {
      changeCircle
        ...
        const focusSource = visaChartsUtils_umd.select(n[i]).classed('vcl-accessibility-focus-source');
        return () => {
          if (focusSource) {
  ...
  onHoverHandler(d, n) {
    ...
    if (this.showTooltip) {
      this.eventsTooltip({ data: d.data.data, evt: visaChartsUtils_umd.event, isToShow: true });
    }
  }
  onMouseOutHandler() {
  ...
  render() {
    ...
    return (index$1.h("div", { class: "o-layout", "data-testid": "outer-layout" }, index$1.h("div", { class: "o-layout--chart", "data-testid": "layout-chart" }, index$1.h(this.topLevel, { "data-testid": "main-title" }, this.mainTitle), index$1.h(this.bottomLevel, { class: "visa-ui-text--instructions", "data-testid": "sub-title" }, this.subTitle), index$1.h("keyboard-instructions", { uniqueID: this.chartID, geomType: 'node', groupName: 'node collection', chartTag: 'circle-packing', width: this.width - (this.margin ? this.margin.right || 0 : 0), isInteractive: this.accessibility.elementsAreInterface, hasCousinNavigation: true, cousinResultOverride: 'Drill up or down a level', disabled: this.suppressEvents &&
        this.accessibility.elementsAreInterface === false &&
        this.accessibility.keyboardNavConfig &&
        this.accessibility.keyboardNavConfig.disabled }), index$1.h("div", { class: "visa-viz-circle-packing-container", "data-testid": "chart-container" }), index$1.h("div", { class: "circle-packing-tooltip vcl-tooltip", "data-testid": "tooltip-container", style: { display: this.showTooltip ? 'block' : 'none' } }), index$1.h("data-table", { uniqueID: this.chartID, isCompact: true, tableColumns: this.tableColumns, data: this.tableData, padding: this.padding, margin: this.margin, hideDataTable: this.accessibility.hideDataTableButton }))));
  }
  init() {
    // reading properties
  ...
  get circlePackingEl() { return index$1.getElement(this); }
...
exports.circle_packing = CirclePacking;
