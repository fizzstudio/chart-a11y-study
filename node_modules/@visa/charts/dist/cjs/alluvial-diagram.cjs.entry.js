...
const point = require('./point-8c60c2dc.js');
...
function link(curve) {
  ...
  link.x = function(_) {
    return arguments.length ? (x = typeof _ === "function" ? _ : constant.constant(+_), link) : x;
  };

  link.y = function(_) {
    return arguments.length ? (y = typeof _ === "function" ? _ : constant.constant(+_), link) : y;
  };
...
// getScopedData,
checkHovered, initTooltipStyle, overrideTitleTooltip, scopeDataKeys, chartAccessors, sankey, sankeyCenter, sankeyJustify, sankeyLeft, sankeyLinkHorizontal, sankeyRight, prepareRenderChange, visaColors, validateAccessibilityProps } = visaChartsUtils_umd.visaChartsUtils_umd;
const AlluvialDiagram = class {
  constructor(hostRef) {
  ...
  componentWillLoad() {
    const chartID = this.uniqueID || 'alluvial-diagram-' + index$1.v4();
  ...
  callSankeyGenerator() {
    ...
    this.nodeCount = max.max(this.preppedData.nodes, d => d.layer);
  }
  renderRootElements() {
    this.svg = visaChartsUtils_umd.select(this.alluvialDiagramEl)
    ...
    this.tooltipG = visaChartsUtils_umd.select(this.alluvialDiagramEl).select('.alluvial-tooltip');
  }
  updateCursor() {
  ...
  setGlobalSelections() {
    ...
    if (this.previousNodeLayers) {
      ...
      this.previousMaxNodeLayer = max.max(updatingNodesWithoutEnteringNodes, d => d.layer);
      this.previousMinNodeLayer = min.min(updatingNodesWithoutEnteringNodes, d => d.layer);
      ...
      this.currentMaxNodeLayer = max.max(updatingNodesWithoutExitingNodes, d => d.layer);
      this.currentMinNodeLayer = min.min(updatingNodesWithoutExitingNodes, d => d.layer);
    }
  ...
  setTestingAttributes() {
    if (this.unitTest) {
      visaChartsUtils_umd.select(this.alluvialDiagramEl)
        .select('.visa-viz-d3-alluvial-container')
  ...
  drawDuplicateClickedLink(inputElement, d) {
    ...
    visaChartsUtils_umd.select(sourceCopy)
    ...
    if (this.innerLinkFillMode === 'source' || this.innerLinkFillMode === 'target') {
      ...
      const validSiblings = visaChartsUtils_umd.select(parentOfParent).selectAll('.link-group')._groups[0];
  ...
  removeTemporaryClickedLinks(root) {
    visaChartsUtils_umd.select(root)
      .selectAll('.duplicated-clicked-link')
      .remove();
  }
  updateLinkStyle() {
  ...
  updateNodeGeometries() {
    ...
    this.updateNodes
      ...
      .duration((_, i, n) => {
      if (visaChartsUtils_umd.select(n[i].parentNode).classed('entering')) {
      ...
      // .duration(this.duration)
      .delay((_, i, n) => {
      return visaChartsUtils_umd.select(n[i].parentNode).classed('entering') ? this.duration / 1.2 : 0;
    })
      .ease(circle.circleIn)
      .attr('opacity', 1
    // d => {
  ...
  drawNodeGeometries() {
    this.updateNodes
      ...
      .duration((_, i, n) => {
      return visaChartsUtils_umd.select(n[i].parentNode).classed('entering') ? this.duration / 3 : this.duration;
    })
      .delay((_, i, n) => {
      return visaChartsUtils_umd.select(n[i].parentNode).classed('entering') ? this.duration / 1.2 : 0;
    })
  ...
  updateNodeLabels() {
    ...
    this.updatingLabels
      ...
      // .duration(this.duration)
      .delay((_, i, n) => {
      return visaChartsUtils_umd.select(n[i]).classed('entering') ? this.duration / 1.2 : 0;
    })
      .attr('opacity', d => {
      if (this.linkConfig.visible) {
        let matchHover = true;
  ...
  drawNodeLabels() {
    ...
    const nodeLabelUpdate = this.updatingLabels
      ...
      .duration((_, i, n) => {
      return visaChartsUtils_umd.select(n[i]).classed('entering') ? this.duration / 3 : this.duration;
    })
      .delay((_, i, n) => {
      return visaChartsUtils_umd.select(n[i]).classed('entering') ? this.duration / 1.2 : 0;
    })
  ...
  onHoverHandler(d, n) {
    ...
    if (this.showTooltip) {
      this.eventsTooltip({ data: d.data, evt: visaChartsUtils_umd.event, isToShow: true });
    }
  }
  onMouseOutHandler() {
  ...
  get alluvialDiagramEl() { return index.getElement(this); }
...
AlluvialDiagram.style = alluvialDiagramCss;

exports.alluvial_diagram = AlluvialDiagram;
