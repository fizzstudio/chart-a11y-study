...
const StackedBarChart = class {
  ...
  componentWillLoad() {
    const chartID = this.uniqueID || 'stacked-bar-chart-' + index$1.v4();
  ...
  prepareScales() {
    ...
    if (this.layout === 'vertical') {
      if (this.interpolating) {
        ...
        this.interpolating.x = band.band()
      ...
      if (this.interpolating) {
        ...
        this.interpolating.x = linear.linear()
          .domain(this.interpolating.oldXDomain)
          .range(this.interpolating.oldXRange);
        this.interpolating.y = band.band()
          .domain(this.interpolating.oldYDomain)
          .range(this.interpolating.oldYRange)
          .padding(this.interpolating.padding);
      }
  ...
  renderRootElements() {
    this.svg = visaChartsUtils_umd.select(this.stackedBarChartEl)
    ...
    this.legendG = visaChartsUtils_umd.select(this.stackedBarChartEl)
      .select('.stacked-bar-legend')
      .append('svg');
    this.tooltipG = visaChartsUtils_umd.select(this.stackedBarChartEl).select('.stacked-bar-tooltip');
    this.references = this.rootG.append('g').attr('class', 'stacked-bar-reference-line-group');
  }
  setTestingAttributes() {
    if (this.unitTest) {
      visaChartsUtils_umd.select(this.stackedBarChartEl)
        .select('.visa-viz-d3-stacked-bar-container')
  ...
  exitGeometries() {
    ...
    this.exitBarWrappers
      ...
      .ease(circle.circleIn)
      .attr('opacity', 0)
      .attr(valueAxis, (_, i, n) => visaChartsUtils_umd.select(n[i]).attr(valueAxis))
      ...
      .attr(valueDimension, (_, i, n) => visaChartsUtils_umd.select(n[i]).attr(valueDimension))
      .attr(ordinalDimension, 0);
  ...
  drawAnnotations() {
    ...
    if (this.data[0][this.groupAccessor] instanceof Date) {
      const maxDate = max.max(this.data, d => d[this.groupAccessor]);
      const minDate = min.min(this.data, d => d[this.groupAccessor]);
      const range = this.layout !== 'horizontal' ? [0, this.innerPaddedWidth] : [this.innerPaddedHeight, 0];
      ordinalScale = time.scaleTime()
  ...
  render() {
    ...
    // be moved into componentWillUpdate (if the stenicl bug is fixed)
    return (index.h("div", { class: `o-layout is--${this.layout} ${theme}` }, index.h("div", { class: "o-layout--chart" }, index.h(this.topLevel, { class: "stacked-bar-main-title vcl-main-title" }, this.mainTitle), index.h(this.bottomLevel, { class: "visa-ui-text--instructions stacked-bar-sub-title vcl-sub-title" }, this.subTitle), index.h("div", { class: "stacked-bar-legend vcl-legend", style: { display: this.legend.visible ? 'block' : 'none' } }), index.h("keyboard-instructions", { uniqueID: this.chartID, geomType: 'bar', groupName: 'stack', chartTag: 'stacked-bar-chart', width: this.width - (this.margin ? this.margin.right || 0 : 0), isInteractive: this.accessibility.elementsAreInterface, hasCousinNavigation: true, disabled: this.suppressEvents &&
        this.accessibility.elementsAreInterface === false &&
        this.accessibility.keyboardNavConfig &&
        this.accessibility.keyboardNavConfig.disabled }), index.h("div", { class: "visa-viz-d3-stacked-bar-container" }), index.h("div", { class: "stacked-bar-tooltip vcl-tooltip", style: { display: this.showTooltip ? 'block' : 'none' } }), index.h("data-table", { uniqueID: this.chartID, isCompact: true, tableColumns: this.tableColumns, data: this.tableData, padding: this.padding, margin: this.margin, hideDataTable: this.accessibility.hideDataTableButton, unitTest: this.unitTest }))));
  }
  init() {
    // reading properties
  ...
  get stackedBarChartEl() { return index.getElement(this); }
...
StackedBarChart.style = stackedBarChartCss;

exports.stacked_bar_chart = StackedBarChart;
