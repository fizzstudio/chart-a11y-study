...
const BarChart = class {
  ...
  componentWillLoad() {
    const chartID = this.uniqueID || 'bar-chart-' + index$1.v4();
  ...
  setColors() {
    ...
    const minValue = min.min(this.preppedData, d => d[this.valueAccessor]);
    const maxValue = max.max(this.preppedData, d => d[this.valueAccessor]);
    this.preparedColors = this.groupAccessor
      ? getColors(colorFromProps, linear.ordinal()
  ...
  prepareScales() {
    ...
    const minBarValue = this.minValueOverride && this.minValueOverride < min.min(this.preppedData, d => d[this.valueAccessor])
      ? this.minValueOverride
      : min.min(this.preppedData, d => d[this.valueAccessor]);
    const maxBarValue = this.maxValueOverride && this.maxValueOverride > max.max(this.preppedData, d => d[this.valueAccessor])
      ? this.maxValueOverride
      : max.max(this.preppedData, d => d[this.valueAccessor]);
    if (this.layout === 'vertical') {
  ...
  drawXGrid() {
    drawGrid(this.gridG, this.innerPaddedHeight, this.innerPaddedWidth, this.x, false, !(!(this.layout === 'vertical') && this.xAxis.gridVisible), this.xAxis.tickInterval, this.duration);
  }
  drawYGrid() {
    drawGrid(this.gridG, this.innerPaddedHeight, this.innerPaddedWidth, this.y, true, !(this.layout === 'vertical' && this.yAxis.gridVisible), this.yAxis.tickInterval, this.duration);
  }
  enterGeometries() {
  ...
  setTestingAttributes() {
    if (this.unitTest) {
      visaChartsUtils_umd.select(this.barChartEl)
  ...
  renderRootElements() {
    this.svg = visaChartsUtils_umd.select(this.barChartEl)
      .select('.visa-viz-d3-bar-container')
      .append('svg')
      .attr('width', this.width)
      .attr('height', this.height)
    ...
    this.legendG = visaChartsUtils_umd.select(this.barChartEl)
      .select('.bar-legend')
      .append('svg');
    this.tooltipG = visaChartsUtils_umd.select(this.barChartEl).select('.bar-tooltip');
  ...
  updateInteractionState() {
    ...
    if (addCollisionClass) {
      const labelsAdded = this.updatingLabels.filter((_, i, n) => visaChartsUtils_umd.select(n[i]).classed('collision-added'));
  ...
  processLabelPosition(selection, runTransition, redrawBitmap, suppressMarkDraw) {
    ...
    selection
      ...
      return this.placement === 'auto' || this.dataLabel.collisionHideOnly ? visaChartsUtils_umd.select(n[i]).style('visibility') : null;
    })
      .attr(`data-${ordinalAxis}`, d => this[ordinalAxis](d[this.ordinalAccessor]))
      .attr(`data-${ordinalDimension}`, this[ordinalAxis].bandwidth())
      .attr(`data-${valueAxis}`, d => this[valueAxis](Math[choice](0, d[this.valueAccessor]))) // this.y(d[this.valueAccessor]))
    ...
    this.bitmaps = placeDataLabels({
      ...
      avoidCollision: {
        ...
        accessors: [this.ordinalAccessor, this.groupAccessor],
        size: [roundTo(this.width, 0), roundTo(this.height, 0)],
  ...
  render() {
    ...
    // be moved into componentWillUpdate (if the stenicl bug is fixed)
    return (index.h("div", { class: `o-layout is--${this.layout} ${theme}` }, index.h("div", { class: "o-layout--chart" }, index.h(this.topLevel, { class: "bar-main-title vcl-main-title" }, this.mainTitle), index.h(this.bottomLevel, { class: "visa-ui-text--instructions bar-sub-title vcl-sub-title" }, this.subTitle), index.h("div", { class: "bar-legend vcl-legend", style: { display: this.legend.visible && this.groupAccessor ? 'block' : 'none' } }), index.h("keyboard-instructions", { uniqueID: this.chartID, geomType: 'bar', groupName: 'bar group', chartTag: 'bar-chart', width: this.width - (this.margin ? this.margin.right || 0 : 0), isInteractive: this.accessibility.elementsAreInterface, hasCousinNavigation: this.groupAccessor ? true : false, disabled: this.suppressEvents &&
        this.accessibility.elementsAreInterface === false &&
        this.accessibility.keyboardNavConfig &&
        this.accessibility.keyboardNavConfig.disabled }), index.h("div", { class: "visa-viz-d3-bar-container" }), index.h("div", { class: "bar-tooltip vcl-tooltip", style: { display: this.showTooltip ? 'block' : 'none' } }), index.h("data-table", { uniqueID: this.chartID, isCompact: true, tableColumns: this.tableColumns, data: this.tableData, padding: this.padding, margin: this.margin, hideDataTable: this.accessibility.hideDataTableButton, unitTest: this.unitTest }))));
  }
  init() {
    // reading properties
  ...
  get barChartEl() { return index.getElement(this); }
...
BarChart.style = barChartCss;

exports.bar_chart = BarChart;
