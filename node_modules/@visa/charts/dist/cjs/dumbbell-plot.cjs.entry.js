...
const DumbbellPlot = class {
  constructor(hostRef) {
    ...
    this.handleMarkerColors = (d, i, n) => {
      ...
      visaChartsUtils_umd.select(n[i]).attr('fill', fillColor);
    ...
    this.handleBarOpacity = (d, i, n) => {
      ...
      visaChartsUtils_umd.select(n[i]).attr('stroke-opacity', opacity);
      return opacity;
    };
  ...
  componentWillLoad() {
    const chartID = this.uniqueID || 'dumbbell-plot-' + index$1.v4();
  ...
  renderRootElements() {
    this.svg = visaChartsUtils_umd.select(this.dumbbellPlotEl)
    ...
    this.legendG = visaChartsUtils_umd.select(this.dumbbellPlotEl)
      .select('.dumbbell-legend')
      .append('svg');
    this.tooltipG = visaChartsUtils_umd.select(this.dumbbellPlotEl).select('.dumbbell-tooltip');
  ...
  setTestingAttributes() {
    if (this.unitTest) {
      visaChartsUtils_umd.select(this.dumbbellPlotEl)
        .select('.visa-viz-d3-dumbbell-container')
  ...
  enterDumbbells() {
    ...
    this.enter
      ...
      ? (d, i, n) => {
        if (d.values) {
          const node = n[i];
          const mouseEvent = visaChartsUtils_umd.event;
  ...
  drawMarkerGeometries() {
    ...
    if (this.marker.type === 'arrow') {
      ...
      this.updateMarkers.selectAll('path').attr('d', (d, i, n) => {
        ...
        const unit = barMult * mult * (barSize + 1);
        const strokeUnit = unit ? 10 / unit : 0;
        visaChartsUtils_umd.select(n[i]).attr('transform', d.focus && this.focusMarker.key && d.referenceIndex === 1
          ? `rotate(180,${5 + strokeUnit / 2},${5 + strokeUnit / 2})`
          : null);
  ...
  drawSeriesLabels(interactionOverride) {
    ...
    const seriesLabelUpdate = this.updateSeriesLabel
      ...
      .style('visibility', (_, i, n) => isAuto || this.seriesLabelDetails.collisionHideOnly ? visaChartsUtils_umd.select(n[i]).style('visibility') : null)
      .attr('fill', this.handleSeriesLabelColors)
  ...
  drawDifferenceLabels(interactionOverride) {
    ...
    const updatingDiffLabels = this.updateDiffLabel
      ...
      .style('visibility', (_, i, n) => isAuto || this.differenceLabel.collisionHideOnly ? visaChartsUtils_umd.select(n[i]).style('visibility') : null)
      ...
      .attr('text-anchor', (_, i, n) => isAuto && visaChartsUtils_umd.select(n[i]).attr('text-anchor')
      ? visaChartsUtils_umd.select(n[i]).attr('text-anchor')
      : this.isVertical && isStandard
        ? 'end'
        : this.isVertical
          ? 'start'
  ...
  drawDataLabels(interactionOverride) {
    ...
    const labelUpdate = this.updateLabelChildren
      .attr('fill', this.handleLabelColors)
      .style('visibility', (_, i, n) => isAuto || this.dataLabel.collisionHideOnly ? visaChartsUtils_umd.select(n[i]).style('visibility') : null)
      .attr('data-translate-x', this.margin.left + this.padding.left)
      .attr('data-translate-y', this.margin.top + this.padding.top)
      .attr('data-use-dx', hideOnly)
      .attr('data-use-dy', hideOnly)
      ...
      visaChartsUtils_umd.select(textElement).attr('data-x', d => (this.isVertical ? this.x(d[this.ordinalAccessor]) : this.x(d[this.valueAccessor]) - d.offset) + xAdjust);
      return this.isVertical
  ...
  findMarkerIndex(data, node, mouseEvent) {
    ...
    const start = +visaChartsUtils_umd.select(node).attr('data-center' + a + '1');
    const end = +visaChartsUtils_umd.select(node).attr('data-center' + a + '2');
  ...
  render() {
    ...
    // be moved into componentWillUpdate (if the stenicl bug is fixed)
    return (index.h("div", { class: "o-layout" }, index.h("div", { class: "o-layout--chart" }, index.h(this.topLevel, { class: "dumbbell-main-title vcl-main-title", "data-testid": "main-title" }, this.mainTitle), index.h(this.bottomLevel, { class: "visa-ui-text--instructions dumbbell-sub-title vcl-sub-title", "data-testid": "sub-title" }, this.subTitle), index.h("div", { class: "dumbbell-legend vcl-legend", style: { display: this.legend.visible ? 'block' : 'none' } }), index.h("keyboard-instructions", { uniqueID: this.chartID, geomType: 'dumbbell', chartTag: 'dumbbell-plot', width: this.width - (this.margin ? this.margin.right || 0 : 0), isInteractive: this.accessibility.elementsAreInterface, disabled: this.suppressEvents &&
        this.accessibility.elementsAreInterface === false &&
        this.accessibility.keyboardNavConfig &&
        this.accessibility.keyboardNavConfig.disabled }), index.h("div", { class: "visa-viz-d3-dumbbell-container" }), index.h("div", { class: "dumbbell-tooltip vcl-tooltip", style: { display: this.showTooltip ? 'block' : 'none' } }), index.h("data-table", { uniqueID: this.chartID, isCompact: true, tableColumns: this.tableColumns, data: this.tableData, padding: this.padding, margin: this.margin, hideDataTable: this.accessibility.hideDataTableButton }))));
  }
  init() {
    // reading properties
  ...
  get dumbbellPlotEl() { return index.getElement(this); }
...
DumbbellPlot.style = dumbbellPlotCss;

exports.dumbbell_plot = DumbbellPlot;
